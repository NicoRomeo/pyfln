...
def _create_header(Nc, TC):
    ...
    http://www.cemyuksel.com/research/yarnmodels/ and
...
def export_closed_BCC(filename, arr_list):
    ...
    See for file format specifications
    http://www.cemyuksel.com/research/yarnmodels/ and
    http://www.cemyuksel.com/cyCodeBase/soln/using_bcc_files.html
...
def readtxt_certificate(filename, sparse=False):
    ...
    If scipy is available, sparse=True defines a scipy.sparse dictionary-of-keys sparse matrix. 
    ...
    sparse: bool, defaults to False
        ...
        numpy array. If scipy is not found, fallbacks to sparse=False.
    ...
    if sparse:
        try:
            ...
            scipy.sparse = None
            sparse = False
            print("Scipy not found. Fallback to numpy array.")
    ...
    with open(filename, 'r') as file:
        ...
        if sparse:
            lnks = scipy.sparse.dok_matrix((Ncurve, Ncurve), dtype=np.int32)
        else:
